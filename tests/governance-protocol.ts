...
describe("governance-protocol", () => {
  ...
  describe("Private Voting (ZK)", () => {
    ...
    it("should enable private voting config for proposal", async () => {
      ...
      // Committee members (Solana pubkeys)
      const committee = [
      ...
      // Committee ElGamal public keys (Ristretto255 points, 32 bytes each)
      const committeeElgamalPubkeys = committee.map(() =>
        Array.from(Buffer.alloc(32)) // Placeholder - real keys from zk-voting-sdk
      );

      // Joint encryption pubkey (Ristretto255 point, 32 bytes)
      const encryptionPubkey = Array.from(Buffer.alloc(32));
    ...
    it("should cast private vote with ElGamal ciphertexts and validity proof", async () => {
      const config = await program.account.governanceConfig.fetch(govConfigPda);
      const proposalId = config.proposalCount.toNumber();
      ...
      // ElGamal ciphertexts: R (32 bytes) || C (32 bytes) = 64 bytes each
      const ctFor = Array.from(Buffer.alloc(64));      // Enc(pk, v_for * weight)
      const ctAgainst = Array.from(Buffer.alloc(64));   // Enc(pk, v_against * weight)
      const ctAbstain = Array.from(Buffer.alloc(64));   // Enc(pk, v_abstain * weight)

      // Vote validity proof: 3 OR proofs (96 bytes each) + 1 sum proof (64 bytes) = 352 bytes
      // Real proofs would be generated by zk-voting-sdk
      const proofData = Array.from(Buffer.alloc(352));

      await program.methods
        .castPrivateVote(ctFor, ctAgainst, ctAbstain, proofData)
        .accounts({
          proposal: privateProposalPda,
          privateVotingConfig: privateVotingPda,
      ...
      // Private vote ciphertexts stored on-chain
      expect(voteRecord.ctFor.length).to.equal(64);
      expect(voteRecord.ctAgainst.length).to.equal(64);
      expect(voteRecord.ctAbstain.length).to.equal(64);
    });
  });
